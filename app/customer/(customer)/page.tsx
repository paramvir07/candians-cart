...
export default async function CustomerPage() {
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex flex-col items-center justify-center h-[60vh] text-center px-4">
          <div className="bg-red-50 text-red-600 p-4 rounded-lg border border-red-100 max-w-md">
            ...
            <p>{response.message || "Please verify your account matches a registered store."}</p>
          </div>
        </div>
      </div>
    );
  ...
  const products = response.products ? JSON.parse(JSON.stringify(response.products)) : [];

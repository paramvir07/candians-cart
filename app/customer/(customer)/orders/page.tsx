...
import { OrderTabs } from "@/components/customer/orderHistory/OrderTabs"
...
const PLACEHOLDER = "https://i.pinimg.com/736x/0d/aa/29/0daa294d95b91e53007b5e472ad6a492.jpg"
  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        {/* Header */}
        <div className="mb-5">
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-950">Order history</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            You have <span className="font-semibold text-gray-900">{PrevOrders.length} orders</span> in total
          </p>
        </div>
        ...
        <div className="mt-6 flex flex-col gap-3">
          {PrevOrders.map((order: OrderWithProductsClient) => {
            const firstProduct = order.products?.[0]
            const thumbUrl = firstProduct?.productId?.images?.[0]?.url ?? PLACEHOLDER
            ...
            return (
              <Card key={order._id.toString()} className="w-full border-border overflow-hidden">
                <CardHeader className="hidden"><CardTitle /></CardHeader>
                <CardContent className="p-0">
                  <Accordion type="single" collapsible>
                    <AccordionItem value={order._id.toString()} className="border-none">
                        ...
                        <div className="flex items-center gap-3 w-full min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between sm:hidden mr-2">
                              <div>
                                <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wide">Order</p>
                                <p className="text-sm font-bold text-foreground tabular-nums">#{orderId}</p>
                              </div>
                              <div className="text-right">
                                <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wide">Total</p>
                                <p className="text-sm font-bold text-primary tabular-nums">CA${fmt(order.cartTotal)}</p>
                              </div>
                            </div>
                            ...
                            <div className="hidden sm:flex items-center gap-10 w-full">
                              <div>
                                <p className="text-[11px] text-muted-foreground font-medium uppercase tracking-wide">Order number</p>
                                <p className="text-sm font-bold text-foreground tabular-nums">#{orderId}</p>
                              </div>
                              <div>
                                <p className="text-[11px] text-muted-foreground font-medium uppercase tracking-wide">Ordered date</p>
                                <p className="text-sm font-bold text-foreground">{date}</p>
                              </div>
                              <div>
                                <p className="text-[11px] text-muted-foreground font-medium uppercase tracking-wide">Items</p>
                                <p className="text-sm font-bold text-foreground">{order.products.length}</p>
                              </div>

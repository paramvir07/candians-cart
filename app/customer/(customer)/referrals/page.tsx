...
import { ReferralPageClient, ReferralDocument } from "@/components/customer/referral/ReferralPageClient";
...
const Page = async () => {
  ...
  if (ReferralNotGenerated) return(
    <>
    <Navbar/>
    <ReferralGenerate customerId={UserData._id.toString()} customerName={UserData.name} />;
    </>
  ) 
  ...
  const referralDoc: ReferralDocument = {
    ...
    code: d.code,
    maxUses: d.maxUses ?? 0,
    expiresAt: d.expiresAt instanceof Date
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <Navbar />
      <ReferralPageClient referralData={referralDoc} userData={userData} />
    </div>
  );
};

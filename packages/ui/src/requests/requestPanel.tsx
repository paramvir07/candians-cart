...
import {
  acceptRequest,
  resolveRequest,
} from "@/actions/admin/Requests/request";
...
const HELP_CATEGORIES: ReportCategory[] = [
  "bug",
  "question",
  "feature",
  "other",
...
const CATEGORY_META: Record<string, CatMeta> = {
  bug: {
    pill: "bg-rose-500 text-white border-rose-600",
    dropdownIcon: "bg-rose-100 text-rose-600 border-rose-200",
    icon: <Bug className="w-3 h-3" />,
    label: "Bug",
  },
  question: {
    pill: "bg-violet-500 text-white border-violet-600",
    dropdownIcon: "bg-violet-100 text-violet-600 border-violet-200",
    icon: <HelpCircle className="w-3 h-3" />,
    label: "Question",
  },
  feature: {
    pill: "bg-cyan-500 text-white border-cyan-600",
    dropdownIcon: "bg-cyan-100 text-cyan-600 border-cyan-200",
    icon: <Sparkles className="w-3 h-3" />,
    label: "Feature",
  },
  other: {
    pill: "bg-slate-500 text-white border-slate-600",
    dropdownIcon: "bg-slate-100 text-slate-600 border-slate-200",
    icon: <MoreHorizontal className="w-3 h-3" />,
    label: "Other",
  },
  "General Inquiry": {
    pill: "bg-slate-600 text-white border-slate-700",
    dropdownIcon: "bg-slate-100 text-slate-600 border-slate-200",
  ...
  "Savings & Pricing": {
    ...
    dropdownIcon: "bg-emerald-100 text-emerald-600 border-emerald-200",
    icon: <Tag className="w-3 h-3" />,
    label: "Savings & Pricing",
  },
  "Gift Wallet & Rewards": {
    pill: "bg-pink-500 text-white border-pink-600",
    dropdownIcon: "bg-pink-100 text-pink-600 border-pink-200",
    icon: <Sparkles className="w-3 h-3" />,
    label: "Gift Wallet & Rewards",
  },
  "Technical Support": {
    pill: "bg-orange-500 text-white border-orange-600",
    dropdownIcon: "bg-orange-100 text-orange-600 border-orange-200",
    icon: <Wrench className="w-3 h-3" />,
    label: "Technical Support",
  },
  Partnership: {
    pill: "bg-indigo-500 text-white border-indigo-600",
    dropdownIcon: "bg-indigo-100 text-indigo-600 border-indigo-200",
...
const STAGE_META = {
  new: {
    ...
    pill: "bg-amber-500 text-white border-amber-600",
    bar: "bg-amber-400",
  },
  accepted: {
    label: "In Progress",
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <ChevronDown
          className={cn(
            "w-3.5 h-3.5 ml-0.5 transition-transform duration-150",
            open && "rotate-180",
          )}
        />
      </button>
          ...
          {options.map((opt) => {
            ...
            return (
              ...
              >
                <span
                  className={cn(
                    "inline-flex items-center justify-center w-5 h-5 rounded-md border text-[11px]",
                    meta?.dropdownIcon,
                  )}
                >
                  {meta?.icon}
...
function HelpCard({
  ...
  return (
    <div
      className={cn(
        "border border-border rounded-xl bg-card transition-colors",
        pending && "opacity-50",
      )}
    >
      <button
      ...
      >
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <ChevronRight
            className={cn(
              "w-4 h-4 text-muted-foreground shrink-0 mt-0.5 transition-transform duration-150",
              expanded && "rotate-90",
            )}
          />
        </div>
            ...
            <button
              onClick={(e) => {
                ...
                startTransition(async () => {
                  onAccept(item._id);
                  await acceptRequest(item._id, "help");
                });
              }}
              disabled={pending}
          ...
          {stage === "accepted" && (
            <button
              onClick={(e) => {
                ...
                startTransition(async () => {
                  onResolve(item._id);
                  await resolveRequest(item._id, "help");
                });
              }}
              disabled={pending}
          ...
          {stage === "new" && (
            <button
              onClick={(e) => {
                ...
                startTransition(async () => {
                  onAccept(item._id);
                  await acceptRequest(item._id, "contact");
                });
              }}
              disabled={pending}
              className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-primary text-primary-foreground hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-40"
          ...
          {stage === "accepted" && (
            <button
              onClick={(e) => {
                ...
                startTransition(async () => {
                  onResolve(item._id);
                  await resolveRequest(item._id, "contact");
                });
              }}
              disabled={pending}
              className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-emerald-600 dark:bg-emerald-500 text-white hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-40"
...
export default function RequestsPanel({
  ...
  const [contactItems, setContactItems] =
    useState<ContactItem[]>(initialContact);

  const helpAccept = useCallback(
    (id: string) =>
      setHelpItems((p) =>
        p.map((i) => (i._id === id ? { ...i, accepted: true } : i)),
      ),
    [],
  );
  const helpResolve = useCallback(
    (id: string) =>
      setHelpItems((p) =>
        p.map((i) =>
          i._id === id ? { ...i, accepted: true, resolved: true } : i,
        ),
      ),
    [],
  );
  const contactAccept = useCallback(
    (id: string) =>
      setContactItems((p) =>
        p.map((i) => (i._id === id ? { ...i, accepted: true } : i)),
      ),
  ...
  const contactResolve = useCallback(
    (id: string) =>
      setContactItems((p) =>
        p.map((i) =>
          i._id === id ? { ...i, accepted: true, resolved: true } : i,
  ...
  const count = (arr: (HelpItem | ContactItem)[], stage: Stage) =>
    arr.filter((i) => getStage(i) === stage).length;

  const hs = {
    new: count(helpItems, "new"),
    inProgress: count(helpItems, "accepted"),
    resolved: count(helpItems, "resolved"),
  };
  const cs = {
    new: count(contactItems, "new"),
    inProgress: count(contactItems, "accepted"),
    resolved: count(contactItems, "resolved"),
  };
  const activeStats = tab === "help" ? hs : cs;

  const filterOptions =
    tab === "help"
      ? (HELP_CATEGORIES as string[])
      : (CONTACT_TOPICS as string[]);
  ...
  const filtered = <T extends HelpItem | ContactItem>(items: T[]): T[] =>
    items
      .filter((i) => {
        ...
        const inView =
          view === "ongoing" ? stage !== "resolved" : stage === "resolved";
        const catMatch =
          filter === null ||
          ("category" in i ? i.category === filter : i.topic === filter);
        return inView && catMatch;
      })
      .sort((a, b) =>
        view === "resolved"
          ? 0
          : { new: 0, accepted: 1, resolved: 2 }[getStage(a)] -
            { new: 0, accepted: 1, resolved: 2 }[getStage(b)],
      );
    ...
    <div className="py-16 text-center">
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-xs text-muted-foreground">
          {view === "resolved"
            ? "Resolved items appear here once marked done."
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        {SUMMARY_CARDS.map((c) => (
          <div
            key={c.label}
            className={cn(
              "rounded-xl sm:rounded-2xl border border-border bg-card shadow-sm p-3 sm:p-4 border-l-4",
              c.border,
            )}
          >
            <div className="flex items-start justify-between mb-2 sm:mb-3">
              ...
              <div
                className={cn("p-1 sm:p-1.5 rounded-lg shrink-0", c.iconCls)}
              >
                {c.icon}
              </div>
            </div>
              ...
              {tab === "help"
                ...
                : contactFiltered.map((item) => (
                    <ContactCard
                      key={item._id}
                      item={item}
                      onAccept={contactAccept}
                      onResolve={contactResolve}
                    />
                  ))}
            </div>
          )}

  ...
  if (draft.minPrice !== undefined || draft.maxPrice !== undefined) {
    ...
    const hi =
      draft.maxPrice !== undefined
        ? `CA$${(draft.maxPrice / 100).toFixed(0)}`
        : `CA$${MAX_PRICE_DOLLARS}+`;
    parts.push(`CA$${lo}–${hi}`);
  }
  if ((draft.taxRates?.length ?? 0) > 0)
    parts.push(
      draft.taxRates!.map((r) => `${(r * 100).toFixed(0)}%`).join("+") + " tax",
    );
...
function PriceRangeSection({
  ...
  const setMin = useCallback(
    (val: number) => {
      if (val < 0) {
        setErrors((e) => ({ ...e, min: "Must be ≥ 0" }));
        setDraft({ ...draft, minPrice: 0 });
      } else if (val >= maxCents) {
        setErrors((e) => ({ ...e, min: "Must be less than max" }));
        setDraft({ ...draft, minPrice: maxCents - 100 });
      } else {
        setErrors((e) => ({ ...e, min: null }));
  ...
  const setMax = useCallback(
    (val: number) => {
      if (val > MAX_PRICE_CENTS) {
        setErrors((e) => ({ ...e, max: `Max is CA$${MAX_PRICE_DOLLARS}` }));
        setDraft({ ...draft, maxPrice: MAX_PRICE_CENTS });
      } else if (val <= minCents) {
        setErrors((e) => ({ ...e, max: "Must be greater than min" }));
        setDraft({ ...draft, maxPrice: minCents + 100 });
      } else {
        setErrors((e) => ({ ...e, max: null }));
...
function MarkupRangeSection({
  ...
  const setMin = useCallback(
    (val: number) => {
      if (isNaN(val) || val < 0) {
        setErrors((e) => ({ ...e, min: "Must be ≥ 0%" }));
        setDraft({ ...draft, markupMin: 0 });
      } else if (val >= markupMax) {
        setErrors((e) => ({ ...e, min: "Must be less than max" }));
        setDraft({ ...draft, markupMin: markupMax - 5 });
      } else {
        setErrors((e) => ({ ...e, min: null }));
        setDraft({ ...draft, markupMin: val });
      }
    },
  ...
  const setMax = useCallback(
    (val: number) => {
      if (isNaN(val) || val > 100) {
        setErrors((e) => ({ ...e, max: "Max is 100%" }));
        setDraft({ ...draft, markupMax: 100 });
      } else if (val <= markupMin) {
        setErrors((e) => ({ ...e, max: "Must be greater than min" }));
        setDraft({ ...draft, markupMax: markupMin + 5 });
      } else {
        setErrors((e) => ({ ...e, max: null }));
        setDraft({ ...draft, markupMax: val });
      }
    },
    ...
    <FilterSection label="Markup %">
      <div className="space-y-3">
        <Slider
          ...
          onValueChange={([min, max]) => {
            setErrors({ min: null, max: null });
            setDraft({ ...draft, markupMin: min, markupMax: max });
          }}
        />
        <RangeInputs
          minVal={markupMin}
...
function FilterBody({
  ...
  const toggleTax = (rate: TaxRate) => {
    ...
    setDraft({
      ...draft,
      taxRates: current.includes(rate)
          ...
          {[
            {
              ...
              onClick: () =>
                setDraft({
                  ...draft,
                  inStock: draft.inStock === true ? undefined : true,
                }),
            },
            {
              label: "Out of stock",
              ...
              onClick: () =>
                setDraft({
                  ...draft,
                  inStock: draft.inStock === false ? undefined : false,
                }),
            },
            {
              label: "Subsidised",
              emoji: "⭐",
              active: draft.subsidised === true,
              onClick: () =>
                setDraft({
                  ...draft,
                  subsidised: draft.subsidised === true ? undefined : true,
                }),
            },
...
export interface ProductFiltersProps {
  ...
  storeId?: string; // Add storeId to fetch category counts tailored to the user/store
}
...
export function ProductFiltersSheet({
  ...
  // Fetch category counts uniquely scoped by role
  useEffect(() => {
    const fetchCounts = async () => {
      // If admin, we don't pass a storeId so it counts all global products.
      // Otherwise, restrict the count to the specific store logic.
      const effectiveStoreId = role === "admin" ? undefined : storeId;
      const res = await getCategoryCounts(effectiveStoreId);
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      ...
      <SheetContent
        ...
        className={cn(
          ...
          isDesktop ? "w-120 h-full" : "h-[92dvh] rounded-t-2xl md:h-[88dvh]",
        )}
      >
        {/* ── Header ── */}

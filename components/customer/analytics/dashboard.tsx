...
import {
  ...
  CreditCard, Banknote, ChevronDown, Calculator,
  CheckCircle2, Clock, Package, ArrowUpRight, Sparkles,
} from "lucide-react";
  ...
  <Card>
    <CardContent className="p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${accent ? "bg-primary/10" : "bg-secondary"}`}>
          <Icon className={`w-4 h-4 ${accent ? "text-primary" : "text-muted-foreground"}`} />
        </div>
      </div>
      <div>
        <div className="flex items-baseline gap-1">
          {!isCount && <span className="text-sm font-semibold text-muted-foreground">CA$</span>}
          <span className={`text-2xl font-black tabular-nums tracking-tight ${accent ? "text-primary" : "text-foreground"}`}>
            {value}
          </span>
        </div>
...
function SavingsCalculator() {
  ...
  const quickPicks = [50, 100, 200, 300, 500];
    ...
    <Card className="overflow-hidden">
      <CardHeader className="pb-4 border-b border-border bg-primary/[0.02]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-base font-bold">Savings Calculator</CardTitle>
            <p className="text-xs text-muted-foreground mt-0.5">
              See how much you could save with our subsidy program — minimum spend of ${MIN_SPEND} required.
            </p>
          </div>
          <Badge variant="secondary" className="ml-auto shrink-0 hidden sm:flex gap-1 text-primary bg-primary/10 border-primary/20">
              ...
              <div className="flex gap-2 flex-wrap">
                {quickPicks.map(v => (
                  <Button
                    ...
                    size="sm"
                    onClick={() => { setSpend(v); setRawInput(String(v)); }}
          ...
          <div className="flex-1 space-y-3 flex flex-col justify-center">
            ...
            <div className="rounded-xl border border-border bg-muted/30 divide-y divide-border overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3">
                <div>
                  <p className="text-xs font-semibold text-foreground">Your total spend</p>
                </div>
                <span className="text-sm font-bold tabular-nums text-foreground">{fmtDollar(spend)}</span>
              </div>
              <div className="flex items-center justify-between px-4 py-3">
                <div>
                  <p className="text-xs font-semibold text-foreground">Discount applied (60%)</p>
                </div>
                <span className={`text-sm font-bold tabular-nums ${eligible ? "text-primary" : "text-muted-foreground"}`}>
                  {fmtDollar(savings)}
                </span>
              </div>
            </div>
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-baseline gap-1.5">
                  ...
                  <span className="text-sm font-semibold" style={{ color: "rgba(255,255,255,0.7)" }}>/ mo</span>
                </div>
              </div>

              <div className="text-right relative z-10">
                <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: "rgba(255,255,255,0.6)" }}>Annually</p>
                <span
    ...
    <div className="max-w-6xl mx-auto px-4 md:px-8 py-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm mt-0.5 text-muted-foreground">Your spending & order analytics</p>
        </div>
      ...
      <div className="grid lg:grid-cols-5 gap-4">
        ...
        <Card className="lg:col-span-3 overflow-hidden">
          <div className="px-5 pt-5 pb-4 flex items-start justify-between">
            <div>
              <p className="text-xs font-semibold mb-1 text-muted-foreground">Total Spending</p>
              <p className="text-3xl font-black tabular-nums text-foreground">CA${fmt(totalSpent)}</p>
              {totalSubsidy > 0 && (
                <div className="flex items-center gap-1 mt-1">
                  <ArrowUpRight className="w-3 h-3 text-primary" />
                  <span className="text-xs font-semibold text-primary">CA${fmt(totalSubsidy)} subsidised</span>
                </div>
              )}
            </div>
        ...
        <Card className="lg:col-span-2 overflow-hidden">
          <div className="px-5 pt-5 pb-4">
            <p className="text-xs font-semibold mb-1 text-muted-foreground">Orders</p>
            <p className="text-3xl font-black tabular-nums text-foreground">{filteredOrders.length}</p>
          ...
          {filteredOrders.length === 0 ? (
            ...
            filteredOrders.slice(0, 7).map((order, i) => {
              ...
              return (
                ...
                >
                  <span className="col-span-1 text-[11px] font-bold tabular-nums text-muted-foreground">#{i+1}</span>
                  <div className="col-span-4 min-w-0">
                    <p className="text-xs font-semibold truncate text-foreground">
                      {allProducts[0]?.productId?.name ?? "Order"}
                      {allProducts.length > 1 && (
                        <span className="ml-1 font-normal text-muted-foreground">+{allProducts.length - 1} more</span>
                      )}
                    </p>
                  ...
                  <span className="col-span-2 text-[10px] text-muted-foreground">{fmtFull(order.createdAt)}</span>
                  <div className="col-span-2 text-right">
                    <p className="text-sm font-black tabular-nums text-primary">CA${fmt(order.cartTotal)}</p>
                    {(order.subsidyUsed ?? 0) > 0 && (
                      <p className="text-[9px] font-semibold text-primary opacity-60">−${fmt(order.subsidyUsed)}</p>
                    )}
                  </div>
                </div>
              );
        ...
        <div className="flex flex-col gap-4">
          ...
          <Card className="overflow-hidden">
            <div
              ...
              style={{ background: "oklch(0.6271 0.1699 149.2138 / 0.03)" }}
            >
              <p className="text-xs font-black uppercase tracking-widest text-muted-foreground">Payment Modes</p>
            </div>
              ...
              paymentBreakdown.map(([mode, count], i) => (
                ...
                >
                  <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 bg-primary/[0.08]">
                    {mode === "card"   ? <CreditCard className="w-3.5 h-3.5 text-primary" /> :
                     mode === "cash"   ? <Banknote   className="w-3.5 h-3.5 text-primary" /> :
                     mode === "wallet" ? <Wallet     className="w-3.5 h-3.5 text-primary" /> :
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-[10px] font-black tabular-nums shrink-0 text-foreground">{count}</span>
                    </div>
                  </div>
                </div>
              ))
          ...
          {totalSubsidy > 0 && (
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-xs font-black uppercase tracking-widest text-primary">Subsidy Earned</p>
              </div>
              <p className="text-2xl font-black tabular-nums text-primary">
                <span className="text-sm font-semibold mr-0.5 opacity-70">CA$</span>
                {fmt(totalSubsidy)}
              </p>
          ...
          <Card className="overflow-hidden">
            <div className="px-5 py-4 border-b border-border">
              <p className="text-[10px] font-black uppercase tracking-widest mb-2 text-muted-foreground">Via Stripe</p>
              <p className="text-lg font-black tabular-nums text-primary">
                <span className="text-xs font-semibold mr-0.5 text-muted-foreground">CA$</span>
                {fmt(totalStripeLoad)}
              </p>
              <p className="text-[10px] mt-0.5 text-muted-foreground">
            ...
            <div className="px-5 py-4">
              <p className="text-[10px] font-black uppercase tracking-widest mb-2 text-muted-foreground">Via Store</p>
              <p className="text-lg font-black tabular-nums text-foreground">
                <span className="text-xs font-semibold mr-0.5 text-muted-foreground">CA$</span>
                {fmt(totalStoreLoad)}
              </p>
              <p className="text-[10px] mt-0.5 text-muted-foreground">{filteredWallet.length} top-up events</p>
            </div>
          </Card>
        </div>
      </div>

      {/* ── Savings Calculator — full width at bottom */}
      <SavingsCalculator />

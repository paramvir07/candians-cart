...
import { ArrowLeft, ShoppingCart, Package, Check, Sparkles, Tag, X, Info, ChevronRight, Gift } from "lucide-react";
...
//
// How it works:
//   - subsidyBudget = packTotal * 0.21  (the free-grocery wallet)
//   - Selected items are covered fully in order until the budget runs out.
//   - subsidyUsed   = cost of covered items (≤ subsidyBudget)
//   - unusedSubsidy = subsidyBudget - subsidyUsed
//   - The customer's effective spending power  = maxPrice + unusedSubsidy
//     (unused subsidy is NOT lost — it boosts what they can spend)
//   - youPay = selectedTotal - subsidyUsed
//     (they pay the full price of uncovered items, nothing deducted from maxPrice)

function allocateSubsidy(
  combo: { name: string; price: number }[],
  checked: boolean[],
  budget: number
): { subsidyUsed: number; unusedSubsidy: number; youPay: number; itemDiscount: number[] } {
...
function SubsidyDialog({
  ...
  onClose: () => void;
  onConfirm: (selected: typeof pkg.combo) => void;
}) {
  ...
  // Effective spending power: maxPrice + any unused subsidy
  const effectiveSpendingPower = +(pkg.maxPrice + unusedSubsidy).toFixed(2);
    ...
    >
      ...
      >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
            ...
            <div>
              <h2 style={{
                ...
                fontSize: 17, color: "#0f172a", margin: 0,
              }}>
                Choose Your Free Items
              </h2>
              <p style={{
                fontFamily: "'DM Sans', sans-serif", fontSize: 12,
                color: "#64748b", margin: 0, marginTop: 1,
              }}>
                {pkg.name} · Your subsidy wallet: ${subsidyBudget}
              </p>
            </div>
          </div>
        ...
        <div style={{ padding: "14px 24px", background: "#fafafa", borderBottom: "1px solid #f1f5f9" }}>
          ...
          {unusedSubsidy > 0 ? (
            <div style={{
              ...
              <p style={{
                fontFamily: "'DM Sans', sans-serif", fontSize: 12,
                color: "#92400e", margin: 0, lineHeight: 1.6, fontWeight: 500,
              }}>
                <strong>${unusedSubsidy.toFixed(2)} subsidy unused</strong> — this gets added to your spending power.
                Your effective budget becomes <strong>${effectiveSpendingPower.toFixed(2)}</strong> instead of ${pkg.maxPrice}.
                Select more items to use up your full subsidy.
              </p>
            </div>
          ) : (
            <p style={{
              fontFamily: "'DM Sans', sans-serif", fontSize: 11,
              color: "#16a34a", margin: "6px 0 0", fontWeight: 600,
            }}>
        ...
        <div style={{
          ...
          <p style={{
            fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: "#15803d",
            margin: 0, lineHeight: 1.6,
          }}>
            Selected items are covered free in order until your ${subsidyBudget} wallet runs out.
            Any unused subsidy <strong>boosts your spending power</strong> — it's never lost.
          </p>
        </div>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "12px 24px 0" }}>
          ...
          {pkg.combo.map((item, i) => {
            ...
            const effectivePrice = checked[i] ? +(item.price - discount).toFixed(2) : item.price;
        ...
        <div style={{
          ...
          {unusedSubsidy > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
              <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#94a3b8" }}>Unused subsidy → spending boost</span>
              <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#d97706", fontWeight: 600 }}>+${unusedSubsidy.toFixed(2)}</span>
            </div>
          )}
          {unusedSubsidy > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
              <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#94a3b8" }}>Effective Spending Power</span>
              <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#0f172a", fontWeight: 700 }}>${effectiveSpendingPower.toFixed(2)}</span>
            </div>
          )}
          <div style={{
            display: "flex", justifyContent: "space-between", alignItems: "center",
            background: "#f0fdf4", borderRadius: 10, padding: "10px 14px", margin: "10px 0 14px",
          }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button
              onClick={() => onConfirm(pkg.combo.filter((_, i) => checked[i]))}
              disabled={checked.every(v => !v)}
              style={{
                flex: 2, padding: "13px 0",
                background: checked.every(v => !v)
                  ? "#e2e8f0"
...
function PackageCard({
  ...
  // "You pay" on the card = pack price (they pay maxPrice, subsidy is on top as free items)
  const youPay = pkg.maxPrice;
  ...
  const handleConfirm = (_items: typeof pkg.combo) => {
    setDialogOpen(false);
    onSelect(pkg.id);
  };
      ...
      >
        ...
        <div style={{ padding: "24px 22px 0" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
            <span style={{
              ...
              <Tag size={11} /> +${subsidy} free groceries
            </span>
            <span style={{
          ...
          <p style={{
            ...
            Use your <strong style={{ color: "#15803d" }}>${subsidy} free grocery credit</strong> on any eligible item — these are just a few examples.
          </p>

          <div style={{ borderRadius: 10, overflow: "hidden", border: "1px solid #f1f5f9", marginBottom: 0 }}>
        ...
        <div style={{ padding: "14px 22px 0", borderTop: "1px solid #f1f5f9", marginTop: 14 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
            ...
            <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: "#16a34a", fontWeight: 700 }}>+${subsidy}</span>
          </div>
          <div style={{
            display: "flex", justifyContent: "space-between", alignItems: "center",
            background: isSelected ? "#f0fdf4" : "#fafafa",
            borderRadius: 10, padding: "12px 14px", transition: "background 0.3s",
          }}>
            <div>
              <span style={{ fontFamily: "'Sora', sans-serif", fontWeight: 700, fontSize: 15, color: "#0f172a" }}>You Pay</span>
              <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: "#94a3b8", marginTop: 1 }}>
                + pick ${subsidy} of free items
              </div>
            </div>
            <span style={{ fontFamily: "'Sora', sans-serif", fontWeight: 800, fontSize: 22, color: "#16a34a", letterSpacing: -0.5 }}>${youPay}</span>
          </div>
        </div>

        <div style={{ padding: "14px 22px 22px" }}>
          <button
            onClick={() => isSelected ? onSelect(pkg.id) : setDialogOpen(true)}
            style={{
              width: "100%",
          ...
          >
            {isSelected
              ? "Remove This Pack"
              : <><ShoppingCart size={17} /> Choose This Pack — +${subsidy} Free</>
            }
          </button>
        </div>
      </div>
      ...
      <main style={{
        ...
        <div style={{
          ...
          <p style={{
            ...
            {" "}as free groceries on top — you pick which items get covered. Unused credit boosts your spending power.
          </p>
          ...
          <div style={{
            ...
            {[
              ...
              { step: "3", label: "Unused credit → more spending power" },
            ].map((s, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {i < 2 && (
                  <ChevronRight size={14} color="#cbd5e1" style={{ margin: "0 6px" }} />
                )}
              </div>
            ))}
          </div>

...
function formatCents(cents: number) {
  if (cents >= 100_000_00) {
    return "$" + (cents / 100 / 1000).toFixed(0) + "k";
  }
  if (cents >= 10_000_00) {
...
export default function StoreStatCards({ stats }: StoreStatCardsProps) {
  const cards = [
    {
      title: "Total orders",
      ...
      sub:
        stats.orderGrowthPercent !== 0
          ? `${stats.orderGrowthPercent > 0 ? "+" : ""}${stats.orderGrowthPercent}% higher than last month`
          : "0%",
    ...
    },
    {
      ...
      sub:
        stats.spendingGrowthPercent !== 0
          ? `${stats.spendingGrowthPercent > 0 ? "+" : ""}${stats.spendingGrowthPercent}% higher than last month`
          : "0%",
    ...
    },
    {
      ...
      sub: stats.pendingPayments === 0 ? "All clear" : "0%",
      growth: 0,
      neutral: true,
      isMoney: true,
      icon: Clock,
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
      {cards.map((card) => {
        ...
        return (
          <div
            key={card.title}
            className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-5 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5"
          >
